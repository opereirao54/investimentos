...
const { db, init, timestamp } = require(
...
async function diagOne(code) {
  ...
  const owners = ownersQ.docs.map((d) => ({
    uid: d.data().uid,
    path: d.ref.path,
  ...
  if (owners.length === 0) {
    ...
    for (const o of owners)
      console.log('  - ' + o.uid + ' (' + (o.email || '?') + ')  [' + o.path + ']');
  }
  ...
  if (!resSnap.exists && owners.length > 0) {
    console.log(
      color('ÓRFÃO: billing aponta para um código sem reserva', 'red') +
        ' — corrija com `--fix ' +
        c +
        '`'
    );
  ...
  if (resSnap.exists && owners.length > 0) {
    ...
    if (mismatch.length === 0) console.log(color('OK: reserva e billing consistentes', 'green'));
      ...
      console.log(
        color('MISMATCH: reserva.uid != billing.uid em ' + mismatch.length + ' caso(s)', 'red')
      );
  }
}
...
async function scanAll() {
  ...
  console.log(
    '  total billing com referralCode:  ' +
      Array.from(inBilling.values()).reduce((a, b) => a + b.length, 0)
  );
  console.log(
    '  ' +
      color('órfãos (billing sem reserva): ' + orphans.length, orphans.length ? 'red' : 'green')
  );
  console.log(
    '  ' +
      color(
        'mismatches (uid divergente): ' + mismatches.length,
  ...
  console.log(
    '  ' +
      color(
        'reservas sem billing dono:    ' + dangling.length,
        dangling.length ? 'yellow' : 'green'
      )
  );
...
async function fixOne(code) {
  ...
  const c = codes.normalize(code);
  if (!codes.isValid(c)) {
    console.log(color('formato inválido', 'red'));
    return;
  }
...
async function main() {
  ...
  if (args[0] === '--fix') {
    if (!args[1]) {
      console.log('Faltou o código depois de --fix');
      process.exit(1);
    }
