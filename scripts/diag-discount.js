...
async function main() {
  ...
  await call(H.subscribe, {
    headers: { authorization: 'Bearer ' + bobTok },
    body: { cpfCnpj: '11122233344', name: 'Bob' },
  });
  ...
  const bobPay = Array.from(asaasState.payments.values()).find(
    (p) => p.subscription === bobB.subscriptionId
  );
  ...
  step(
    2,
    'EXISTENTE: Carla criou conta SEM cupom, depois recebe um cupom de amigo e tenta aplicar'
  );
  ...
  log(
    '1ª init (sem cupom): customerId =',
    carlaB1.customerId,
    '· discount =',
    carlaB1.recurringDiscountPercent
  );
  ...
  log(
    '2ª init (com cupom): customerId =',
    carlaB2.customerId,
    '· discount =',
    carlaB2.recurringDiscountPercent
  );
  log(
    'billing devolvido pela 2ª init =',
    JSON.stringify({
  ...
  check(
    carlaB2.recurringDiscountPercent === 10,
    'EXPECTATIVA: 2ª init aplica o cupom retroativamente'
  );

  // Carla assina
  await call(H.subscribe, {
    headers: { authorization: 'Bearer ' + carlaTok },
    body: { cpfCnpj: '22233344455', name: 'Carla' },
  });
  ...
  const stat = await call(H.status, {
    method: 'GET',
    headers: { authorization: 'Bearer ' + bobTok },
  });
  ...
  log(
    'status.billing.subscriptionBaseValueCents   =',
    stat.body.billing.subscriptionBaseValueCents
  );
  check(
    stat.body.billing.recurringDiscountPercent === 10,
    'status devolve recurringDiscountPercent=10'
  );
  check(stat.body.billing.monthlyPriceCents === 1500, 'status devolve monthlyPriceCents=1500');
  check(
    stat.body.billing.subscriptionBaseValueCents === 1350,
    'status devolve subscriptionBaseValueCents=1350'
  );
  log('');
