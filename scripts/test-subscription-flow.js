...
async function main() {
  ...
  const betoMe1 = await call(H.me, {
    method: 'GET',
    headers: { authorization: 'Bearer ' + betoTok },
  });
  ...
  await call(H.init, {
    headers: { authorization: 'Bearer ' + dianaTok },
    body: { referralCode: carlaCode },
  });
  const dianaB = store.docs.get('users/diana_uid/billing/account');
  check(
    dianaB.recurringDiscountPercent === 10,
    'cupom registrado mesmo sem pagar (vale para quando pagar)'
  );
  ...
  check(
    d2.body.access.reason === 'trial_expired',
    'reason: trial_expired (cupom só dá DESCONTO ao pagar)'
  );

  step('D3', 'Carla NÃO recebeu crédito da Diana (ninguém pagou)');
  const carlaCreditsAfterD = Array.from(store.docs.keys()).filter((k) =>
    k.startsWith('users/carla_uid/billing/account/credits/')
  );
  log('créditos da Carla =', carlaCreditsAfterD.length);
  check(
    carlaCreditsAfterD.length === 1,
    'apenas o crédito vindo do Carlos (que pagou) — Diana não gera crédito'
  );
  ...
  await call(H.webhook, {
    ...
    body: {
      ...
      payment: { ...annaPay, status: 'OVERDUE' },
    },
  });
  ...
  step(
    'E2',
    'Defesa C5: forçar subscriptionStatus=ACTIVE com lastPaymentStatus=OVERDUE — não restaura acesso'
  );
  ...
  annaForChg.subscriptionStatus = 'ACTIVE';
  annaForChg.lastPaymentStatus = 'CONFIRMED';
  await call(H.webhook, {
    headers: { 'asaas-access-token': TEST_TOKEN },
    body: {
      id: 'evt_anna_chg',
      event: 'PAYMENT_CHARGEBACK_REQUESTED',
      payment: { ...annaPay, status: 'CHARGEBACK_REQUESTED' },
    },
  });
  ...
  await call(H.webhook, {
    ...
    body: {
      ...
      payment: {
        id: 'stale_' + carlosPay.id,
        subscription: carlosForCancel.subscriptionId,
        status: 'CONFIRMED',
        value: 13.5,
      },
    },
  ...
  step(
    'F3',
    '/me após cancelamento durante o trial: mantém trial até vencer (comportamento do produto)'
  );
  const f3a = await call(H.me, {
    method: 'GET',
    headers: { authorization: 'Bearer ' + carlosTok },
  });
  check(
    f3a.body.access.status === 'trial',
    'durante o trial: ainda trial (cancelamento só toma efeito após o trial vencer)'
  );

  step(
    'F4',
    'Trial expira após cancelamento → mantém acesso paid_period (Carlos pagou há minutos, dentro dos 30 dias)'
  );
  ...
  const g1 = H.computeAccess({
    trialEndsAt: makeTimestamp(Date.now() + 86400 * 1000),
    subscriptionStatus: null,
  });
  check(g1.status === 'trial', 'trial');

  step('G2', 'computeAccess: ACTIVE + CONFIRMED → active');
  const g2 = H.computeAccess({
    subscriptionStatus: 'ACTIVE',
    lastPaymentStatus: 'CONFIRMED',
    lastPaidAt: makeTimestamp(Date.now() - 1000),
  });
  ...
  const g5 = H.computeAccess({
    subscriptionStatus: 'ACTIVE',
    lastPaymentStatus: 'CHARGEBACK_REQUESTED',
  });
  ...
  step(
    'G9',
    'computeAccess: INACTIVE + lastPaidAt < 30d → active/paid_period (cancelou mas pagou no ciclo)'
  );
  ...
  step(
    'G11',
    'computeAccess: trial vivo + INACTIVE + lastPaidAt < 30d → trial (trial vence o paid_period)'
  );
