...
│   ├── appliquei-rendimento.js      # Classic — valor de mercado da carteira numa data passada
...
- **O rendimento tem data** (`web/appliquei-rendimento.js`): a DRE mostra quanto saiu do bolso (custo) e quanto isso virou (mercado) no fim de CADA mês da janela. Renda fixa e previdência são exatas em qualquer data — juros compostos capitalizados dia a dia. Renda variável usa o **fechamento da época**, de `?op=history`; sem a série (rede fora, sem sessão) cai para a cotação de hoje e a célula ganha `~`, porque valorar a posição de março com o preço de setembro devolve o ganho de hoje aplicado ao passado. O gráfico de evolução usa o mesmo caminho — era essa a limitação que ele declarava em comentário.
