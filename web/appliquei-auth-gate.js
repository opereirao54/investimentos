...
function ensureEmailVerifyBanner() {
  ...
  if (resendBtn)
    resendBtn.addEventListener('click', function () {
      ...
      u.sendEmailVerification({ url: location.origin + '/app' })
        .then(function () {
          if (typeof mostrarToast === 'function')
            mostrarToast(
              'E-mail reenviado. Pode levar alguns minutos — confira também a caixa de spam.',
              'sucesso',
              8000
            );
      ...
      u.reload()
        ...
        .then(function () {
          if (u.emailVerified) {
            ...
            if (typeof mostrarToast === 'function')
              mostrarToast('E-mail verificado com sucesso.', 'sucesso');
          } else {
            if (typeof mostrarToast === 'function')
              mostrarToast(
                'Ainda não vimos a verificação. Clique no link do e-mail e tente novamente.',
                'aviso'
              );
          }
        })
...
/**
 * Cabeçalho do gate. O título era fixo em "Bem-vindo" em TODOS os estados —
 * inclusive sobre "Confirme seu e-mail" e sobre "Não encontramos sua conta",
 * onde dar boas-vindas soa errado. Título e subtítulo passam a andar juntos.
 */
function setCabecalho(titulo, sub) {
  var t = $('authGateTitulo');
...
window.appliqueiAuthSetModo = function (modo) {
  ...
  setCabecalho(
    login ? 'Bem-vindo de volta' : 'Criar sua conta',
    login ? 'Entre para continuar de onde parou' : '7 dias grátis, sem cartão de crédito'
  );
...
function mapAuthErr(err) {
  if (!err || !err.code)
    return err && err.message ? String(err.message) : 'Não foi possível concluir.';
  var c = err.code;
  ...
  if (c === 'auth/popup-blocked')
    return 'O navegador bloqueou o popup. Permita popups para este site ou tente novamente.';
  if (c === 'auth/account-exists-with-different-credential')
    return 'Já existe uma conta com este e-mail usando outro método (senha). Entre com senha primeiro e depois vincule o Google em Configurações.';
  if (
    c === 'auth/unauthorized-domain' ||
    /requests-from-referer-.*-are-blocked/i.test(c) ||
    /requests-from-referer-.*-are-blocked/i.test(err.message || '')
  ) {
    return (
      'Este endereço (' +
      location.hostname +
      ') não está autorizado no Firebase. Use o domínio oficial do app ou peça ao administrador para adicioná-lo em Authentication › Settings › Authorized domains.'
    );
  }
...
window.appliqueiAuthSubmit = function () {
  ...
  if (!senha || senha.length < 6)
    return window.appliqueiAuthErr('Informe a senha (mínimo 6 caracteres).');
  var btn = $('authBtnSubmit');
  if (btn) {
    btn.disabled = true;
  }
  ...
  if (reg) {
    ...
    if (cupom) {
      try {
        sessionStorage.setItem('appliquei_pending_referral', cupom);
      } catch (_) {}
    }
  }
  var p = reg
    ? fb.auth.createUserWithEmailAndPassword(email, senha).then(function (cred) {
        ...
        try {
          return cred.user
            .sendEmailVerification({ url: location.origin + '/app' })
            .then(function () {
              return cred;
            });
        } catch (_) {
...
window.appliqueiAuthResend = function () {
  ...
  u.sendEmailVerification({ url: location.origin + '/app' })
    .then(function () {
      if (status) {
        status.innerHTML =
          'E-mail reenviado. Pode levar alguns minutos — <strong>confira também a pasta de spam</strong>.';
    ...
    .catch(function (err) {
      if (status) {
        status.textContent = mapAuthErr(err);
...
window.appliqueiAuthVerifyCheck = function () {
  ...
  u.reload()
    ...
    .finally(function () {
      if (btn) {
        btn.disabled = false;
        btn.textContent = 'Já verifiquei — entrar';
      }
    });
};
...
window.appliqueiAuthEsqueciSenha = function () {
  ...
  var email = (($('authEmail') && $('authEmail').value) || '').trim();
  if (!email)
    return window.appliqueiAuthErr('Informe o e-mail para enviarmos o link de redefinição.');
  window.appliqueiAuthErr('');
  fb.auth
...
window.appliqueiContinuarOffline = function () {
  window.appliqueiAuthErr(
    'O modo offline foi descontinuado. Crie uma conta para iniciar a avaliação gratuita de 7 dias.'
  );
};
try {
...
window.appliqueiSidebarAuthClick = function () {
  ...
  showGate();
  if (!fb || !fb.ready) {
    setCabecalho('Não conseguimos iniciar', 'A autenticação não carregou neste navegador');
    setAuthPanel('noconf');
  } else {
...
function initAppliqueiAuth() {
  ...
  showGate();
  setAuthPanel('load');
  setCabecalho('Bem-vindo', 'Um instante — estamos preparando tudo');
    ...
    if (user) {
      ...
      if (user.emailVerified === false) {
        if (emailVerifyGateHard()) {
          ...
          showGate();
          setCabecalho('Quase lá', 'Só falta confirmar o seu e-mail');
          setAuthPanel('verify');
          refreshSidebar();
          return;
        }
